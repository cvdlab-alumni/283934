...
var model03 = STRUCT([model02,forest1,forest2]);

//EXERCISE 4

function generateBuilding(x,y){
	return STRUCT([T([0,1,2])([x,y,0.12]),CUBOID([Math.random()/2,Math.random()/3,Math.random()/2])]);
}

function disposeBuildings(startX,startY){
	var village = new Array();
	for(i=startX;i<startX+3;i++){
		for(j=startY;j<startY+3;j++){
			var randomize = Math.random();
			var building = generateBuilding(i,j);
			village.push(building);
		}
	}
	return STRUCT([COLOR([1.39,1.21,0.94])(STRUCT(village))]);
}

var village1 = disposeBuildings(1,-4);
var village2 = disposeBuildings(1,-8);
var village3 = disposeBuildings(1,-12);

var model04 = STRUCT([model03,village1,village2,village3]);

//EXERCISE 5

function disposeRoads(startX,startY){
	var roads = new Array();
	for(i=startX;i<startX+3;i++){
		for(j=startY;j<startY+3;j++){
			var road =  STRUCT([T([0,1,2])([i,j,0.15]),CUBOID([0.4,0.3])]);
			roads.push(road);
		}
	}
	return STRUCT([COLOR([1.05,1.39,1.05])(STRUCT(roads))]);
}

var roads1 = disposeRoads(1,-4);
var roads2 = disposeRoads(1,-8);
var roads3 = disposeRoads(1,-12);

var model = STRUCT([model04,roads1,roads2,roads3]);
DRAW(model);
